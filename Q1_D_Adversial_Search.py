...
class PuzzleState:
    ...
    def get_successors(self):
        ...
        # Define moves: (delta_row, delta_col, action_name)
        moves = [
    ...
    def get_path(self):
        """Reconstruct path from start to this state"""
        path = []
        current = self
...
def validate_board(board: List[List]) -> bool:
    """Validate that the board is a valid 8-puzzle configuration"""
    if len(board) != 3:
        return False
...
def read_from_file(filename: str = 'input.txt') -> Optional[List[List]]:
    """Read start state from input.txt file"""
    try:
    ...
    def alpha_beta(self, state: PuzzleState, depth: int, alpha: float, beta: float, 
        ...
        if is_max_player:
            ...
            for successor in successors:
                ...
                # Beta cutoff: MIN won't allow MAX to reach this value
                if beta <= alpha:
                    self.pruned_count += 1
            ...
            for successor in successors:
                ...
                # Alpha cutoff: MAX won't allow MIN to reach this value
                if beta <= alpha:
                    self.pruned_count += 1
                    break  # Prune remaining branches
...
def simulate_adversarial_game(start: PuzzleState, goal: PuzzleState, 
    ...
    while move_count < max_moves:
        ...
        move_count += 1
        is_max_turn = (move_count % 2 == 1)  # Odd moves: MAX, Even moves: MIN
        
        print(f"\n{'='*70}")
        print(f"Move {move_count}: {'MAX (Sorter)' if is_max_turn else 'MIN (Glitch)'}")
        print(f"{'='*70}")
        
        # Create search instance and run algorithm directly
...
def compare_minimax_alphabeta(start: PuzzleState, goal: PuzzleState, max_depth=6):
    ...
    # Handle None values for best_action (when already at goal)
    ...
    # Display comparison in organized format
    print("\n" + "="*70)
...
def main():
    """Main execution for adversarial search"""
    print("\n" + "#"*70)
    print("# QUESTION 1D: ADVERSARIAL SEARCH - MINIMAX & ALPHA-BETA")
    print("#"*70 + "\n")
    
    # Fixed goal state (standard 8-puzzle goal)
    goal_board = [
        [1, 2, 3],
        [4, 5, 6],
        [7, 8, 'B']
    ...
    # 1. Compare Minimax vs Alpha-Beta on single move
    print("\n" + "="*70)
    ...
    # 2. Simulate adversarial game with Minimax
    print("\n\n" + "="*70)
    ...
    # 3. Simulate adversarial game with Alpha-Beta
    print("\n\n" + "="*70)
